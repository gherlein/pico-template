...
static inline uint8_t bcdnum(uint8_t bcd) { return ((bcd / 16) * 10) + (bcd % 16); }

// convert number to BCD
static inline uint8_t numbcd(uint8_t num) { return ((num / 10) * 16) + (num % 10); }

bool dev_ds3231_setdatetime(i2c_inst_t* i2c, datetime_t* dt) {
    // check datetime
    if (!sys_check_datetime(dt)) return false;
...
bool dev_ds3231_getdatetime(i2c_inst_t* i2c, datetime_t* dt) {
    ...
    if (sys_i2c_rbuf_reg(i2c, DEV_DS3231_ADDR, 0, dt_buffer, sizeof(dt_buffer)) != sizeof(dt_buffer)) return false;
...
bool dev_ds3231_setrtc(i2c_inst_t* i2c) {
    ...
    if (!dev_ds3231_getdatetime(i2c, &dt)) return false;
